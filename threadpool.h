...
pool.sunmitTask(std::make_shared<Task>());
*/
...
private:
    ...
    int threadSizeThreadhold_;                 //线程数量上限阈值  cached模式用

    int initThreadSize_;                        // 初始线程数量  fix模式用
    std::atomic_int curThreadSize_;                    //记录当前线程池里线程的数量
    std::atomic_int idleThreadSize_;   //记录空闲线程的数量

    std::queue<std::shared_ptr<Task>> taskQue_; // 任务队列
    std::atomic_int taskSize_;                  // 任务数量
    int taskQueMaxThreadHold_;               // 任务队列数量的上限阈值

    std::mutex taskQueMtx_;            // 锁，保证任务队列的线程安全
    ...
    std::atomic_bool isPoolRunning_;     //当前线程池的启动状态,标识线程池状态的变量可能在多个线程使用，需要保证一致性
